...
crazySHA::crazySHA(int num, std::string file) {
  ...
  for (int i = 0; i < numOfThreads; ++i) {
    thrArray.emplace_back(std::thread([&]() { startSearch(); }));
  }
}
    ...
    if (hash_hex_str.substr(60, 4) == "0000") {
      ...
      double duration =
          std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start)
              .count();
      start = stop;
      BOOST_LOG_SEV(slg, boost::log::trivial::info)
          << std::endl
          << "source: " << std::hex << std::stol(proimage) << std::dec
          << " hash: " << hash_hex_str << " duration: " << duration
          << " thread: " << std::this_thread::get_id() << std::endl;
      nlohmann::json j = {{"source", proimage},
